/*
 * ================================================================
 *  GEAR SHIFT RACE  –  ATmega2560 / NEWTC DM-MSTYLE-EXT
 *  Microprocessor Application Final Project
 * ================================================================
 *
 *  버튼
 *    PJ0 (D15)  CLUTCH    외부 풀다운, active HIGH
 *    PJ1 (D14)  SHIFT UP  외부 풀다운, active HIGH
 *    PD0 (D21)  ACCEL     내부 풀업,   active LOW
 *
 *  LCD  (PORTL)
 *    PL0–PL3 DB4–DB7  /  PL4 E  /  PL5 RS
 *
 *  DC 모터  (L293D)
 *    PD7 Enable  /  PG2 Direction  /  PH6 OC2B PWM  (Timer2)
 *
 *  서보 1 – RPM 게이지  (Timer1 Fast PWM 50Hz)
 *    PB5 OC1A  D11    900RPM → 0°  /  6800RPM → 180°
 *
 *  서보 2 – 속도계      (Timer4 Fast PWM 50Hz)
 *    PH4 OC4B  D7     0km/h → 0°  /  150km/h → 180°
 *
 *  UART0  9600 baud
 *    50ms마다 "S:XXX,R:XXXX,G:X\n" 전송  →  engine_sound.py 연동
 *
 * [ 게임 방법 ]
 *  1. 아무 버튼 → Player 1 대기
 *  2. ACCEL → 레이스 시작 (400m)
 *  3. CLUTCH 누른 채 UP으로 기어 변경, 떼면 체결
 *  4. 400m 완주 후 기록 표시 → Player 2 차례
 *  5. 결과 화면에서 아무 버튼 → 처음으로
 *
 * [ 최적 업시프트 포인트 ]
 *  1→2단: ~35–38 km/h  |  2→3단: ~60–68 km/h  |  3→4단: ~90–100 km/h
 *  4→5단: ~125–135 km/h  |  5→6단: ~158–168 km/h
 * ================================================================
 */
...
/* ── 버튼 인덱스 ────────────────────────────────────────────── */
#define BTN_CLUTCH  0   /* PJ0  D15  외부풀다운  active HIGH */
#define BTN_UP      1   /* PJ1  D14  외부풀다운  active HIGH */
#define BTN_ACCEL   2   /* PD0  D21  내부풀업    active LOW  */
#define BTN_COUNT   3
...
/* ── 서보 펄스 범위 (Code3 검증값, 0.5µs/tick) ─────────────── */
#define SERVO_MIN  1400U   /* ~0°   (0.7 ms) */
#define SERVO_MAX  4600U   /* ~180° (2.3 ms) */
...
/* ================================================================
 *  Timer3 – ms 카운터  (prescaler 1024, overflow)
 *  Timer1이 서보1 PWM에 사용되므로 Timer3 사용
 * ================================================================ */
...
/* ================================================================
 *  Timer2 – DC 모터 PWM  (OC2B = PH6)
 *  OC2B(IN1)=HIGH→브레이크, LOW→전진  →  OCR2B 낮을수록 빠름
 * ================================================================ */
...
/* ================================================================
 *  Timer1 – 서보1 RPM 게이지  (OC1A = PB5 = D11)
 *  Code3와 완전 동일: Fast PWM mode14, prescaler 8, ICR1=39999
 * ================================================================ */
...
/* ================================================================
 *  Timer4 – 서보2 속도계  (OC4B = PH4 = D7)
 *  Timer1과 동일 설정: Fast PWM mode14, prescaler 8, ICR4=39999
 * ================================================================ */
...
/* ================================================================
 *  UART0 – 9600 baud  →  engine_sound.py
 *  포맷: "S:XXX,R:XXXX,G:X\n"  (50ms 주기)
 * ================================================================ */
...
static void buttons_init(void) {
    DDRJ &= ~0x03;           /* PJ0(CLUTCH), PJ1(UP) 입력 */
    clear_bit(DDRD, 0);
    set_bit(PORTD, 0);       /* PD0(ACCEL) 내부풀업 ON */
...
static void outputs_reset(void) {
    motor_set_spd(255);   /* OCR2B=255 → IN1 HIGH = L293D 브레이크 */
    OCR1A = SERVO_MIN;
    OCR4B = SERVO_MIN;
}
